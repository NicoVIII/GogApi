...
module Program =
    let printHelp () =
        ...
        printfn
            "Account/getFilteredGames <search> [<feature> [<language> [<sort> [<system>]]]]"
        printfn ""
    ...
    let handleGetFilteredGames search feature language sort system page authentication =
        let feature =
            if feature = "-"
            then None
            else feature |> GameFeature.fromString |> Some

        let language =
            if language = "-" then None else language |> Language.fromString |> Some

        let search =
            if search = "-" then None else search |> Some

        let sort =
            if sort = "-" then None else sort |> Sort.fromString |> Some

        let system =
            if system = "-" then None else system |> OS.fromString |> Some
    ...
    let rec handleCommand authentication =
        ...
        | [ "Account/getFilteredGames"; NonEmptyString search; NonEmptyString feature;
            ...
            |> handleCommand
        | [ "Account/getFilteredGames"; NonEmptyString search; NonEmptyString feature;
            NonEmptyString language; NonEmptyString sort ] ->
            handleGetFilteredGames search feature language sort "-" "-" authentication
            |> handleCommand
        | [ "Account/getFilteredGames"; NonEmptyString search; NonEmptyString feature;
            NonEmptyString language; NonEmptyString sort; NonEmptyString system ] ->
            handleGetFilteredGames search feature language sort system "-" authentication
            |> handleCommand
        | [ "Account/getFilteredGames"; NonEmptyString search; NonEmptyString feature;
            NonEmptyString language; NonEmptyString sort; NonEmptyString system; NonEmptyString page ] ->
