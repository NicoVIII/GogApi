...
/// FSharp.MetadataFormat handles XML documentation in a weird way:
/// It creates HTML but if you only provide a summary before the
/// p tag is an annoying br tag...
/// This is removed by this method
let removeLeadingBreak (html: string) =
    if html.StartsWith("</br>") then html.Substring(5) else html

/// Short version of removeLeadingBreak
let bybr = removeLeadingBreak
...
let formatMember (m: Member) =
    ...
    let hasCustomOp =
        m.Attributes
        |> List.exists (fun a ->
            a.FullName = "Microsoft.FSharp.Core.CustomOperationAttribute")
            ...
            [ h2 [] [ !!t.Name ]
              ...
              table []
                  [ tr []
                    ...
                    if not t.Constructors.IsEmpty
                    then tr [] [ td [ ColSpan 3. ] [ b [] [ !! "Constructors" ] ] ]
                    yield! t.Constructors |> List.map formatMember

                    if not t.InstanceMembers.IsEmpty
                    then tr [] [ td [ ColSpan 3. ] [ b [] [ !! "Instance Members" ] ] ]
                    yield! t.InstanceMembers |> List.map formatMember

                    if not t.RecordFields.IsEmpty
                    then tr [] [ td [ ColSpan 3. ] [ b [] [ !! "Record Fields" ] ] ]
                    yield! t.RecordFields |> List.map formatMember

                    if not t.StaticMembers.IsEmpty
                    then tr [] [ td [ ColSpan 3. ] [ b [] [ !! "Static Members" ] ] ]
                    yield! t.StaticMembers |> List.map formatMember

                    if not t.StaticParameters.IsEmpty
                    then tr [] [ td [ ColSpan 3. ] [ b [] [ !! "Static Parameters" ] ] ]
                    yield! t.StaticParameters |> List.map formatMember

                    if not t.UnionCases.IsEmpty
                    then tr [] [ td [ ColSpan 3. ] [ b [] [ !! "Union Cases" ] ] ]
...
let generate' (ctx: SiteContents) =
    ...
    | Some all ->
        ...
        |> List.collect (fun n ->
            ...
            let modules =
                n.Modules |> Seq.map (generateModule ctx)
