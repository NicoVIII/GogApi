              ...
              table []
                  [ tr []
                    ...
                    if not t.InstanceMembers.IsEmpty then
                        tr [] [ td [ ColSpan 3. ] [ b [] [ !!"Instance Members" ] ] ]
...
let generate' (ctx: SiteContents) =
    ...
    let namespaces = generatorOutput.AssemblyGroup.Namespaces |> List.map (generateNamespace ctx)

    let modules =
        match allModules with
        | Some allModules -> allModules |> Seq.map (generateModule ctx)
        | _ -> Seq.empty

    let types =
        match allTypes with
        | Some allTypes -> allTypes |> Seq.map (generateType ctx)
        | _ -> Seq.empty
...
let generate (ctx: SiteContents) (projectRoot: string) (page: string) =
    try
        generate' ctx |> List.map (fun (n, b) -> n, (Layout.render ctx b))
    with ex ->
