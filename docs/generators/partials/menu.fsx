...
let menu (ctx: SiteContents) (page: string) =
    ...
    let all =
        ctx.GetValues<Apirefloader.AssemblyEntities>()

    let content = ctx.GetValues<Contentloader.Post>()

    let siteInfo =
        ctx.TryGetValue<Globalloader.SiteInfo>().Value
    ...
    let menuHeader =
        [ if hasExplenations then
            li
                [ Id "menu-explanations"
                  if group = Some Contentloader.Explanation
                  then Class "dd-item menu-group-link menu-group-link-active"
                  else Class "dd-item menu-group-link" ] [ a [] [ !! "Explanation" ] ]
          if hasTutorials then
              li
                  [ Id "menu-tutorials"
                    if group = Some Contentloader.Tutorial
                    then Class "dd-item menu-group-link menu-group-link-active"
                    else Class "dd-item menu-group-link" ] [ a [] [ !! "Tutorials" ] ]
          if hasHowTos then
              li
                  [ Id "menu-howtos"
                    if group = Some Contentloader.HowTo
                    then Class "dd-item menu-group-link menu-group-link-active"
                    else Class "dd-item menu-group-link" ] [ a [] [ !! "How-To Guides" ] ]
          li
              [ Id "menu-refs"
                if group = None
                then Class "dd-item menu-group-link menu-group-link-active"
                else Class "dd-item menu-group-link" ] [ a [] [ !! "API References" ] ] ]

    let renderExpls =
        ul
            [ Id "submenu-explanations"
    ...
    let renderRefs =
        ul
            [ Id "submenu-refs"
              if group = None then Class "submenu submenu-active" else Class "submenu" ]
            [ for r in all ->
                li []
                    [ a
    ...
    nav [ Id "sidebar" ]
        [ div [ Id "header-wrapper" ]
              [ div [ Id "header" ] [ h2 [ Id "logo" ] [ !!siteInfo.title ] ]
                div [ Class "searchbox" ]
